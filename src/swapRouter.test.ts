import { BigintIsh, CurrencyAmount, ETHER, Percent, Token, TradeType, WETH9 } from 'udonswap-core'
import { Pair, Route as V2Route, Trade as V2Trade } from 'udonswap-v2-sdk'
import {
  encodeSqrtRatioX96,
  FeeAmount,
  nearestUsableTick,
    ...
    describe('ETH input', () => {
      describe('single-hop exact input (v2 + v3)', () => {
        describe('different trade configurations result in identical calldata', () => {
          ...
          const amountIn = CurrencyAmount.fromRawAmount(ETH, JSBI.BigInt(100))

          const v2Trade = V2Trade.exactIn(new V2Route([pair_1_WETH], ETH, token1), amountIn)
      ...
      describe('single-hop exact output (v2 + v3)', () => {
        describe('different trade configurations result in identical calldata', () => {
          ...
          const v2Trade = V2Trade.exactOut(new V2Route([pair_1_WETH], ETH, token1), amountOut)
          const v3Trade = V3Trade.fromRoute(
      ...
      describe('multi-hop exact input (v2 + v3)', () => {
        describe('different trade configurations result in identical calldata', () => {
          ...
          const v2Trade = V2Trade.exactIn(new V2Route([pair_1_WETH, pair_0_1], ETH, token0), amountIn)
          /// mixedRouteTrade mirrors V2Trade
          const mixedRouteTrade = MixedRouteTrade.fromRoute(
            new MixedRouteSDK([pair_1_WETH, pair_0_1], ETH, token0),
            amountIn,
            TradeType.EXACT_INPUT
          )
          const v3Trade = V3Trade.fromRoute(
            new V3Route([pool_1_WETH, pool_0_1], ETH, token0),
            amountIn,
            TradeType.EXACT_INPUT
          )
      ...
      describe('multi-hop exact output (v2 + v3)', () => {
        describe('different trade configurations result in identical calldata', () => {
          ...
          const v2Trade = V2Trade.exactOut(new V2Route([pair_1_WETH, pair_0_1], ETH, token0), amountOut)
          const v3Trade = V3Trade.fromRoute(
            new V3Route([pool_1_WETH, pool_0_1], ETH, token0),
            amountOut,
            TradeType.EXACT_OUTPUT
      ...
      describe('high price impact with ETH input to result in refundETH being appended to calldata', () => {
        ...
        const v2Trade = V2Trade.exactIn(new V2Route([pair_1_WETH], ETH, token1), amountIn)
        const v3Trade = V3Trade.fromRoute(
          new V3Route([pool_1_WETH_slippage], ETH, token1),
          amountIn,
          TradeType.EXACT_INPUT
        )
    ...
    describe('ETH output', () => {
      describe('single-hop exact input (v2 + v3)', () => {
        describe('different trade configurations result in identical calldata', () => {
          ...
          const v2Trade = V2Trade.exactIn(new V2Route([pair_1_WETH], token1, ETH), amountIn)
      ...
      describe('single-hop exact output (v2 + v3)', () => {
        describe('different trade configurations result in identical calldata', () => {
          ...
          const amountOut = CurrencyAmount.fromRawAmount(ETH, JSBI.BigInt(100))

          const v2Trade = V2Trade.exactOut(new V2Route([pair_1_WETH], token1, ETH), amountOut)
          const v3Trade = V3Trade.fromRoute(
      ...
      describe('multi-hop exact input (v2 + v3)', () => {
        describe('different trade configurations result in identical calldata', () => {
          ...
          const v2Trade = V2Trade.exactIn(new V2Route([pair_0_1, pair_1_WETH], token0, ETH), amountIn)
          /// mixedRouteTrade mirrors v2Trade
          const mixedRouteTrade = MixedRouteTrade.fromRoute(
            new MixedRouteSDK([pair_0_1, pair_1_WETH], token0, ETH),
            amountIn,
            TradeType.EXACT_INPUT
          )
          const v3Trade = V3Trade.fromRoute(
            new V3Route([pool_0_1, pool_1_WETH], token0, ETH),
            amountIn,
            TradeType.EXACT_INPUT
          )
      ...
      describe('multi-hop exact output (v2 + v3)', () => {
        describe('different trade configurations result in identical calldata', () => {
          ...
          const v2Trade = V2Trade.exactOut(new V2Route([pair_0_1, pair_1_WETH], token0, ETH), amountOut)
          const v3Trade = V3Trade.fromRoute(
            new V3Route([pool_0_1, pool_1_WETH], token0, ETH),
            amountOut,
            TradeType.EXACT_OUTPUT
